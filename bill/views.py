...
# ==========================================
# 0. HELPER: GET OR CREATE STORE PROFILE
# ==========================================
def get_store_profile():
    """Ensures a single store profile always exists to prevent DB relational errors."""
...
def login_view(request):
    ...
    if request.user.is_authenticated:
        # Check plan even for already-logged-in users
        store = get_store_profile()
        try:
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            # --- Plan check after login ---
            store = get_store_profile()
            try:
                sub = store.subscription
                if not sub.is_active:
                    # Logged in but plan expired/no plan
                    if user.role == 'admin':
                        # Admin can go buy a plan
                        return redirect('plans_page')
                    else:
                        # Cashier/staff — log them out, show message
                        logout(request)
...
def profile_view(request):
    """Renders the User Profile and Shop Information settings page."""
    store = get_store_profile()
    ...
    if request.method == 'POST':
        ...
        # 2. Update User details (Email & Alt Phone)
        request.user.email = request.POST.get('email', request.user.email)
        request.user.phone = request.POST.get('alt_phone', request.user.phone) # Using user.phone for alt phone
        request.user.save()
...
def dynamic_dashboard_view(request):
    """Parses session RBAC and aggregates contexts for specific role dashboards."""
    user = request.user
    today = timezone.now().date()
    store = get_store_profile()
    
    context = {'role': user.role, 'user': user, 'branch_context': store}
    ...
    if user.role == 'admin':
        ...
        # 2. Pending Payments (NEW)
        pending_invs = Invoice.objects.filter(status='Pending')
        ...
        # 3. Total Products & Categories (NEW)
        context['total_products_count'] = Product.objects.count()
        context['total_categories_count'] = Category.objects.count()

        # 4. Sales Analytics Chart (Last 7 Days)
        chart_labels = []
        chart_sales = []
        chart_revenue = [] 
        ...
        for i in range(6, -1, -1):
            ...
            day_invs = Invoice.objects.filter(created_at__date=target_date, status='Paid')
        ...
        context['low_stock_products'] = Product.objects.filter(stock_quantity__lte=low_stock_threshold)

        # 6. Recent Activity
        context['recent_activities'] = Invoice.objects.all().order_by('-created_at').select_related('cashier', 'customer')[:5]

        return render(request, 'pages/admin_portal.html', context)

    elif user.role == 'cashier':
        # 1. Base Queries for Today and Yesterday
        yesterday = today - timedelta(days=1)
        today_invoices = Invoice.objects.filter(created_at__date=today)
        yesterday_invoices = Invoice.objects.filter(created_at__date=yesterday, status='Paid')

        # 2. Today's Sales & Percentage Difference
        ...
        context.update({
            ...
            'recent_activities': today_invoices.order_by('-created_at')[:10]  # Only show today's bills
        })
...
def pos_invoice_transaction_api(request):
    """Ingests frontend POS state payloads and updates ledgers."""
    ...
    try:
        ...
        # Ensure single store profile exists for the Ledger
        store = get_store_profile()

        # Convert incoming JSON floats to strict Decimals safely
        ...
        # --- ITEMS CREATION & STOCK DEDUCTION ---
        for single_item in items:
            product_obj = get_object_or_404(Product, id=single_item['product_id'])
            order_qty = int(single_item['quantity'])
            
            # Deduct stock directly from Product model
        ...
        if customer_inst:
            # Both are now Decimal types, so += will work perfectly
            customer_inst.total_purchases += grand_total_dec
            customer_inst.save()
...
def billing_pos(request):
    """Renders the POS Billing Engine."""
...
def inventory_list(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'add_product':
            ...
            if new_cat_name and new_cat_name.strip() and cat_id == 'NEW_CATEGORY':
                new_category_obj, created = Category.objects.get_or_create(name=new_cat_name.strip())
    ...
    context = {
        ...
        'low_stock_count': low_stock_count,       # Added to context
        'out_of_stock_count': out_of_stock_count  # Added to context
    }
...
def sales_report(request):
    ...
    # 1. Handle Dynamic Date Range Filter
    try:
    ...
    # 2. Filter data dynamically based on selected date range
    invoices = Invoice.objects.filter(status='Paid', created_at__date__gte=start_date)
    all_invoices = Invoice.objects.filter(created_at__date__gte=start_date).order_by('-created_at')
    ...
    # Provide empty state for Chart.js to prevent crashes
    if not cat_labels: 
    ...
    context = {
        ...
        'recent_invoices': all_invoices, # Passes raw invoices for the table
...
@login_required
def invoice_list(request):
    # Restricted to Admin and Cashier only
    if request.user.role not in ['admin', 'cashier']:
        return redirect('dashboard')

    # Single store system: Fetch all invoices directly
    invoices = Invoice.objects.all().order_by('-created_at')
    ...
    # 2. Serialize data for the frontend table and Modals
    invoice_data = []
    for inv in invoices:
        items = []
        for item in inv.items.all():
    ...
    context = {
        'invoices': invoice_data,
        # Branch contexts completely removed
        'kpi': {
            'total_count': total_count,
            'total_amount': total_amount,
            'paid_count': paid_count,
    ...
    if request.method == 'POST':
        ...
        # --- Handle User Creation ---
            ...
            try:
                ...
                # Count existing non-admin users (cashiers, managers etc.)
